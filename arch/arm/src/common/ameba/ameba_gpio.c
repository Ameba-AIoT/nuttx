...
static struct ameba_gpio_dev_s *g_int_dev[AMEBA_GPIO_NPORTS][32];

/* Set once each port's NuttX interrupt vector has been attached + enabled. */

static bool g_port_attached[AMEBA_GPIO_NPORTS];

/* Per-port NVIC vector, from the chip's <ameba_gpio_chip.h>. */

static const int g_ameba_gpio_irq[AMEBA_GPIO_NPORTS] = AMEBA_GPIO_PORT_IRQS;
static spinlock_t g_ameba_gpio_lock = SP_UNLOCKED;
                                  ...
                                  enum gpio_pintype_e pintype)
{
  ...
  if (init.mode == AMEBA_GPIO_MODE_INT)
    {
      irqstate_t flags = spin_lock_irqsave(&g_ameba_gpio_lock);

      GPIO_INTStatusClearEdge(AMEBA_PIN_PORT(priv->pin));
      GPIO_INTConfig(priv->pin, AMEBA_DISABLE);

      spin_unlock_irqrestore(&g_ameba_gpio_lock, flags);
    }
}
  ...
  while (status != 0)
    {
      ...
      struct ameba_gpio_dev_s *priv = g_int_dev[port][num];
...
static void ameba_gpio_port_attach(uint8_t pin)
{
  ...
  flags = spin_lock_irqsave(&g_ameba_gpio_lock);

  if (!g_port_attached[port])
    {
      int irq = g_ameba_gpio_irq[port];

      irq_attach(irq, ameba_gpio_interrupt, (void *)(uintptr_t)port);
      up_enable_irq(irq);
      g_port_attached[port] = true;
    }
  ...
  if (interrupt)
    {
      ...
      g_int_dev[AMEBA_PIN_PORT(pin)][AMEBA_PIN_NUM(pin)] = priv;
      ...
      if (interrupt)
        {
          ...
          g_int_dev[AMEBA_PIN_PORT(pin)][AMEBA_PIN_NUM(pin)] = NULL;
        }
